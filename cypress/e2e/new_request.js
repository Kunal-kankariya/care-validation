...
it('user create a new request', ()=>{
    ...
    cy.get('.q-item__section').contains('New Request').should('be.visible').click({force:true});
    cy.get('body').click()
    ...
    cy.get('.q-date__calendar-days').find('.q-date__calendar-item--in').contains('2').click({force:true});
    cy.contains('Continue').should('be.enabled').click();
    ...
    cy.get('[data-testid="submit-form-dialog-submit-button"]').should('be.visible').click();
    cy.contains('Thank you for your submission').should('exist');
    cy.contains('Close').should('be.visible').click();
    cy.get('[aria-label="Menu"]').should('be.visible').click();
    cy.get('.q-item__section').contains('My Requests').should('be.visible').click({force:true});
    cy.get('body').click()
